...
class AccommodationCartItem extends Component
{
    ...
    {
        $this->itemPriceSum = $this->one->{$this->price_field} * (Carbon::parse($this->end_date)->diffInDays(Carbon::parse($this->start_date)) + 1);
    }
    ...
    public function updateDuration($option_id, $duration_id)
    {
        ...
        if ($durations) {
            ...
            $this->selected_options[$option_id]['price'] = $durations[$found_key]->{$this->price_field};
    ...
    public function updateOption($id, $checked)
    {
        ...
        if ($this->selected_options[$id]['option']->is_durations && $durations && count($durations) > 0) {
            ...
            $this->selected_options[$id]['price'] = $this->selected_options[$id]['option']->{$this->price_field};
        }
    ...
    {
        return view('livewire.accommodation-cart-item');
    }
}
