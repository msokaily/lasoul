...
@endphp
@section('pageTitle') 
    {{ucwords(__('siderbar.'.request('type')))}} | {{ucwords(__('siderbar.'.$page_name))}}
@endsection
@section('title') 
    {{ucwords(__('siderbar.'.$page_name))}} - {{ucwords(__('siderbar.'.request('type')))}}
@endsection
@if(isAdmin())
    @section('filter') 
        <!--begin::Actions-->
            ...
            <table id="kt_table_accounts" class="table table-row-bordered table-striped">
                    ...
                    @forelse($items as $item)
                        <tr class="odd gradeX" id="tr-{{$item->id}}">
                            <td>{{$x++}}</td>
                            <td> <a style="color: black" href="{{ route('admin.' . $page_name . '.edit', [$item->id]) }}?type={{request('type')}}">{{$item->title}}</a></td>
