...
@endsection
@section('title') 
    {{ucwords(__('siderbar.'.$page_name))}} {{ $cat ? 'In ( '.$cat->name.' ) Category' : '' }}
@endsection
@if(isAdmin())
    @section('filter') 
        <!--begin::Actions-->
            ...
            <div class="card-header border-0 pt-6 search_div">
                ...
                <div class="card-toolbar">
                    ...
                    <div class="d-flex justify-content-end align-items-center">
                        <button type="button" class="btn btn-success submit-sort">{{__("common.save_sort")}}</button>
                    </div>
                    <!--end::Group actions-->
                </div>
                <!--end::Card toolbar-->
            ...
            <form class="save-sort-form" method="post" action="{{route("admin.$page_name.sort")}}">
                ...
                <table id="kt_table_accounts" class="table table-row-bordered table-striped">
                        ...
                        <tr>
                            ...
                            <th> {{ucwords(__('common.parent_cat'))}}</th>
                        ...
                        @forelse($items as $item)
                            <tr class="odd gradeX" id="tr-{{$item->id}}">
                                ...
                                <td> <a style="color: black" href="{{ route('admin.' . $page_name . '.edit', [$item->id]) }}">{{$item->name}}</a></td>
                                <td class="center">{{$item->parent->name ?? '-'}}</td>
                                <td class="center">{!! $item->status == 1 ? '<span style="color: green;">Active</span>' : '<span style="color: red;">InActive</span>' !!}</td>
                                ...
                                <td>
                                    ...
                                    <div class="" style="">
                                        ...
                                        <!--begin::Menu item-->
                                            <a href="{{ route('admin.products.index') }}?cat={{$item->id}}"
                                                class="menu-link px-3 btn btn-sm btn-dark">{{__('common.products')}}</a>
                                        <!--end::Menu item-->
                                        @endif
                                        @if($item->children && count($item->children) > 0)
                                        <!--begin::Menu item-->
                                            <a href="{{ route('admin.categories.index') }}?cat={{$item->id}}"
                                                class="menu-link px-3 btn btn-sm btn-warning">{{__('common.sub_categories')}}</a>
                                        <!--end::Menu item-->
                                        @endif
                                    </div>
                                    <!--end::Menu-->
