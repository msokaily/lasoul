...
@endsection
@section('title') 
    {{ucwords(__('siderbar.'.$page_name))}} - {{ucwords(__('siderbar.'.request('type', 'apartments')))}}
@endsection
@if(isAdmin())
    @section('filter') 
        <!--begin::Actions-->
        <div class="d-flex align-items-center gap-2 gap-lg-3" style="justify-content: flex-end;">
            <!--begin::Top Buttons-->
                <div class="m-0">
                    <!--begin::Add user-->
                    <a href="{{ route('admin.' . $page_name . '.create') }}?type={{request('type')}}" class="btn btn-sm btn-primary">{{__("common.create_new")}}</a>
                    <!--end::Add user-->
                </div>
            <!--end::Top Buttons-->
        </div>
        ...
        <div class="portlet-body">
            ...
            <table id="kt_table_accounts" class="table table-row-bordered table-striped">
                    ...
                    <tr>
                        ...
                        <th> {{ucwords(__('common.number_s'))}}</th>
                        <th> {{ucwords(__('common.location'))}}</th>
                    ...
                    @forelse($items as $item)
                        <tr class="odd gradeX vertical-mid" id="tr-{{$item->id}}">
                            <td>{{$x++}}</td>
                            <td>
                                <div class="d-flex align-items-center">
                                    <!--begin::Thumbnail-->
                                    <a href="{{ route('admin.' . $page_name . '.edit', [$item->id]) }}?type={{request('type')}}" class="symbol symbol-50px">
                                        <span class="symbol-label" style="background-image:url({{$item->image}});"></span>
                                    </a>
                                    <!--end::Thumbnail-->
                                    <div class="ms-5">
                                        <!--begin::Title-->
                                        <a href="{{ route('admin.' . $page_name . '.edit', [$item->id]) }}?type={{request('type')}}" class="text-gray-800 text-hover-primary fs-5 fw-bold" data-kt-ecommerce-product-filter="product_name">{{$item->title}}</a>
                                        <!--end::Title-->
                                    </div>
                                </div>
                            </td>
                            ...
                            <td> {{$item->price}}</td>
                            <td>
                                <span class="badge @if($item->status == 1) badge-light-success @else badge-light-danger @endif" id="label-{{$item->id}}">@if($item->status == 1)
                                    {{__('common.yes')}}
                                @else 
                                    {{__('common.no_')}} 
                                @endif
                            ...
                            <td>
                                ...
                                <div class="" style="">
                                    <!--begin::Menu item-->
                                        <a href="{{ route('admin.' . $page_name . '.edit', [$item->id]) }}?type={{request('type')}}"
